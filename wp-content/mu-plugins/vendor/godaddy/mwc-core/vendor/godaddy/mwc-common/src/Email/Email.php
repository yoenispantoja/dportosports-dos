...
class Email implements EmailContract
{
    ...
    public function setTo($value) : Email
    {
        ...
        $value = is_string($value) ?
            implode(',', array_filter(array_map('trim', explode(',', $value)))) :
            array_filter(array_map('trim', $value));

        // for backwards compatibility, we want to allow an empty $value to bypass validation.
        if (! empty($value)) {
            $invalidEmails = ValidationHelper::findInvalidEmailRecipients($value);

            if (! empty($invalidEmails)) {
                throw new InvalidArgumentException(
                    __('Invalid recipient value to set for email: '.implode(', ', $invalidEmails).' is not a valid email address.', 'mwc-common')
                );
            }
        }
