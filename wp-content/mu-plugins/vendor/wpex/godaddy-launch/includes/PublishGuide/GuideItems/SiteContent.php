...
class SiteContent extends GuideItemAbstract {
	 ...
	 * Class constructor.
	 *
	 * Note: Allows for deep linking to the add new page screen with gdl_action=add-page query param.
	 */
	public function __construct() {
		$gdl_action = filter_input( INPUT_GET, 'gdl_action', FILTER_DEFAULT );

		if ( ! $gdl_action || 'add-page' !== htmlspecialchars( $gdl_action ) ) {
			return;
		}

		wp_safe_redirect( admin_url( 'post-new.php?post_type=page&tooltip=siteContent' ) );
		exit;
	}
