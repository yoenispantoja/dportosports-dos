...
class PaymentTransactionAdapter implements DataSourceAdapterContract
{
    ...
    public function convertFromSource() : ChargeRequest
    {
        ...
        $request = (new ChargeRequest())->setBody($transactionData);

        // Use the idempotency key from the transaction if available
        if ($idempotencyKey = $this->source->getIdempotencyKey()) {
            $request->setPoyntRequestId($idempotencyKey);
        }
