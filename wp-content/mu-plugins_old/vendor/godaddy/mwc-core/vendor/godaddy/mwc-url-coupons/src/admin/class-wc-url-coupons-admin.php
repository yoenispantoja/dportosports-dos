...
class WC_URL_Coupons_Admin {
	...
	public function add_coupon_options( $coupon_id, $coupon ) {
		...
		<div class="options_group">
			...
			/**
			 * Allows for adding custom fields to the coupon options.
			 *
			 * @since 2.16.1
			 *
			 * @param int $coupon_id The shop coupon ID.
			 * @param \WC_Coupon $coupon The shop coupon object.
			 */
			do_action( 'wc_url_coupons_options_discount_links', $coupon->get_id(), $coupon );
			?>
		</div>
		<?php
	}
	...
	public function update_coupons( $options ) {
		...
		$coupons[ $options['coupon_id'] ] = [
			'url'                => is_string($options['unique_url']) ? strtolower( $options['unique_url'] ) : '',
			'prefix'             => is_string($options['url_prefix']) ? strtolower( $options['url_prefix'] ) : '',
