...
use GoDaddy\WordPress\MWC\Core\WooCommerce\Exceptions\FailedOrderException;
use GoDaddy\WordPress\MWC\Core\WooCommerce\Exceptions\InvalidOrderException;
use GoDaddy\WordPress\MWC\Core\WooCommerce\Exceptions\InvalidTokenException;
use GoDaddy\WordPress\MWC\Core\WooCommerce\Exceptions\MissingTokenException;
...
class PreOrderIntegration extends AbstractTokenizedIntegration
{
    ...
    protected function getPaymentMethodForReleasePayment(WC_Order $wooOrder) : AbstractPaymentMethod
    {
        ...
        if (! $paymentMethodRemoteId) {
            throw new MissingTokenException('Payment token is missing.');
        }
        ...
        if ($paymentMethodId = $wooOrder->get_meta("{$this->orderPaymentMetaKey}_paymentMethod_id")) {
            ...
            if ($paymentMethod->getRemoteId() !== $paymentMethodRemoteId || (int) $paymentMethod->getCustomerId() !== (int) $wooOrder->get_user_id()) {
                throw new InvalidTokenException('Payment token is invalid.');
            }
    ...
    public function processReleasePayment($wooOrder)
    {
        try {
            ...
            if ('success' !== ArrayHelper::get($result, 'result')) {
                throw new FailedOrderException(ArrayHelper::get($result, 'message', ''));
            }
