...
class NEWSMUNCH_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'newsmunch-pro' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'newsmunch-pro' );
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Title','newsmunch-pro' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Subtitle','newsmunch-pro' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_subtitle2_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Subtitle 2','newsmunch-pro' ), $this->id, 'customizer_repeater_subtitle2_control' ),
						...
						if($this->customizer_repeater_subtitle3_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Subtitle 3','newsmunch-pro' ), $this->id, 'customizer_repeater_subtitle3_control' ),
						...
						if($this->customizer_repeater_subtitle4_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Subtitle 4','newsmunch-pro' ), $this->id, 'customizer_repeater_subtitle4_control' ),
						...
						if($this->customizer_repeater_subtitle5_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Subtitle 5','newsmunch-pro' ), $this->id, 'customizer_repeater_subtitle5_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Description','newsmunch-pro' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_text2_control==true){
                            $this->input_control(array(
                                'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Button Label','newsmunch-pro' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_button2_control==true){
                            $this->input_control(array(
                                'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Button Label second','newsmunch-pro' ), $this->id, 'customizer_repeater_button2_control' ),
						...
						if($this->customizer_repeater_button_text_control){
							...
							'newsmunch-pro'), $this->id, 'customizer_repeater_button_text_control'),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Link','newsmunch-pro' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_link2_control){
                            $this->input_control(array(
                                'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'link','newsmunch-pro' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_link3_control){
                            $this->input_control(array(
                                'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'link 3','newsmunch-pro' ), $this->id, 'customizer_repeater_link3_control' ),
						...
						if($this->customizer_repeater_video_url_control){
							...
							'newsmunch-pro'), $this->id, 'customizer_repeater_video_url_control'),
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Color','newsmunch-pro' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Shortcode','newsmunch-pro' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						if($this->customizer_repeater_designation_control==true){
							$this->input_control(array(
								'label' => apply_filters('newsmunch_repeater_input_labels_filter', esc_html__( 'Designation','newsmunch-pro' ), $this->id, 'customizer_repeater_designation_control' ),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php printf( __( 'Delete %s', 'newsmunch-pro' ), $this->boxtitle );	?>
						</button>
			...
			<div class="customizer-repeater-general-control-repeater-container">
				...
				<div class="customizer-repeater-box-content-hidden">
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'newsmunch-pro' ); ?>
					</button>
				</div>
			</div>
			<?php
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','newsmunch-pro'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = '', $class=''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','newsmunch-pro'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'newsmunch-pro' ),
	                sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'newsmunch-pro' ) )
                ); ?>
            </span>
		...
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','newsmunch-pro')?>
            </span>
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button <?php if($class="newsmunch_overlimit") { echo esc_attr('newsmunch-uploading-img-btn');}?> <?php echo esc_attr($auto);?>" value="<?php esc_attr_e( 'Upload Image','newsmunch-pro' ); ?>" />
		</div>
		<?php
	}
	...
	private function image_control2($value = '', $show = '', $class='', $auto='', $sections=''){ 
		...
		<div class="customizer-repeater-image2-control">
            <span class="customize-control-title">
                <?php esc_html_e('Image 2','newsmunch-pro')?>
            </span>
	...
	<span class="customize-control-title">
	<?php esc_html_e('Background Type','newsmunch-pro'); ?>
	</span>
	<select class="customizer-repeater-bg-type">
		...
		<?php esc_html_e('Video','newsmunch-pro') ?>
		</option>	
	...
	private function icon_type_choice($value='customizer_repeater_icon', $newsmunch_limit=''){ ?>
		<span class="customize-control-title">
            <?php esc_html_e('Image type','newsmunch-pro');?>
        </span>
		<select class="customizer-repeater-image-choice  <?php echo esc_attr($newsmunch_limit);?>">
			<option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','newsmunch-pro'); ?></option>
			<option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','newsmunch-pro'); ?></option>
			<option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','newsmunch-pro'); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control($value = '', $newsmunch_limit='', $newsmunch_type_with_id=''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'newsmunch-pro' ); ?></span>
		<?php
		if(!empty($value)) {
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) { ?>
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link team_linkdata_<?php echo $newsmunch_limit;?> <?php echo esc_attr($newsmunch_type_with_id);?>"
					       placeholder="<?php esc_attr_e( 'Link', 'newsmunch-pro' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'newsmunch-pro' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary "><?php esc_html_e( 'Add Icon', 'newsmunch-pro' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
				foreach ( $social_repeater as $social_icon ) {
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        ...
						        } ?>"><?php esc_html_e( 'Remove Icon', 'newsmunch-pro' ); ?></button>
					</div>
					<?php
				} ?>
			...
			<button class="social-repeater-add-social-item button-secondary <?php echo esc_attr($newsmunch_limit);?> <?php echo esc_attr($newsmunch_type_with_id);?>"><?php esc_html_e( 'Add Icon', 'newsmunch-pro' ); ?></button>
			<?php
		}
	}
}
...
/**
  ...
  * @return string
  */
 function newsmunch_repeater_labels( $string, $id, $control ) {
	if ( $id === 'newsmunch_slider_option' ) {
		if ( $control === 'customizer_repeater_link_control' ) {
			return esc_html__( 'Link 1','newsmunch-pro' );
		}
		
		if ( $control === 'customizer_repeater_link2_control' ) {
			return esc_html__( 'Link 2','newsmunch-pro' );
		}
		
		if ( $control === 'customizer_repeater_subtitle2_control' ) {
			return esc_html__( 'Form Title','newsmunch-pro' );
		}
	}
 ...
 add_filter( 'newsmunch_repeater_input_labels_filter','newsmunch_repeater_labels', 10 , 3 );