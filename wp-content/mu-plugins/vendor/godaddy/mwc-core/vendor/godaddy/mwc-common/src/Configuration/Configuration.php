...
class Configuration
{
     ...
     * @return array|void
     */
    protected static function load($directories) : array
    {
    ...
    public static function set(string $key, $value = null)
    {
        ...
        if (self::hasUpdatedConfigurations([$key => $value])) {
            ...
            /*
             * Do not persist configuration cache.
             *
             * When the configuration cache is loaded (on every request) {@see Configuration::load()} it is compared
             * against the contents of the configuration files
             * {@see Configuration::getDirectoryContents() and Configuration::hasUpdatedConfigurations}.
             *
             * If Configuration::set() persists it has the effect of invalidating the ConfigurationCache because
             * any new values set() would always differ from the values in the configuration files.
             *
             * Additionally, in an environment where object caching is disabled (ex. MWP, currently) a `set()` will
             * result in a database write and it is likely that many of writes will occur since `set()` may be called
             * through the request for various reason resulting in many differing version of the configuration being
             * written to the database {@see set_transient() and update_option()}.
             *
             */
            Cache::configurations()->set($values, false);
        }
    }
