...
class ExtensionsController extends AbstractController
{
    ...
    {
        $payload = TypeHelper::arrayOfStringsAsKeys($request->get_json_params());
    ...
    {
        $version = ArrayHelper::getStringValueForKey($payload, 'version.version');

        if (version_compare($version, TypeHelper::string($extension->getInstalledVersion(), ''), '!=')) {
