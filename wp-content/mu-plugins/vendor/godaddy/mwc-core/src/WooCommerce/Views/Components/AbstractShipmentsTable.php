...
class AbstractShipmentsTable implements RenderableContract
{
    ...
    protected function getPackagesDataFromShipment(ShipmentContract $shipment) : array
    {
        ...
        return array_map(function (PackageContract $package) use ($providerLabel) {
            return [
                'providerLabel'  => $this->getProviderLabel($package, $providerLabel),
    ...
    /**
     * Attempts to fetch the provider label from the package, and falls back to the default if it is not set.
     *
     ...
     */
    protected function getProviderLabel(PackageContract $package, string $default) : string
    {
        if ($package->getShippingRate() && ! empty($package->getShippingRate()->getCarrier()->getLabel())) {
            return $package->getShippingRate()->getCarrier()->getLabel();
        }
