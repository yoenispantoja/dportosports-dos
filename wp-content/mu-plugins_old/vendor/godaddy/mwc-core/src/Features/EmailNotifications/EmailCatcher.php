...
class EmailCatcher implements ConditionalComponentContract
{
    ...
    /**
     * Maybe intercepts email handling through the pre-sending WP Mail filter.
     *
     * @internal callback for `pre_wp_mail` hook in {@see wp_mail()}
     * @see EmailCatcher::addHooks()
     *
     * @param bool|mixed|null $shortCircuitReturn
     * @param array<string, mixed>|mixed $emailAttributes
    ...
    public function maybeSendEmail($shortCircuitReturn, $emailAttributes)
    {
        ...
        try {
            $emailAttributes = TypeHelper::array($emailAttributes, []);
            ...
            (new RenderableEmail($this->getEmailRecipients($emailAttributes)))
                ...
                ->setSubject(TypeHelper::string(ArrayHelper::get($emailAttributes, 'subject'), ''))
                ->setHeaders($headers)
                ->setContentType($this->getContentType($headers))
                ->setBody(TypeHelper::string(ArrayHelper::get($emailAttributes, 'message'), ''))
    ...
    protected function getEmailRecipients($emailAttributes) : array
    {
        ...
        if (is_string($recipients)) {
            ...
            $recipients = TypeHelper::arrayOfStrings(TypeHelper::ensureArray($recipients));
        }
