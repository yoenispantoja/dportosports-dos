...
class ExceptionHandler
{
    ...
    public static function render(Configuration $config, Throwable $exception): void
    {
        ...
        if ($config->debug || wp_installing() || defined('WP_ADMIN')) {
            $message[] = "<p><code>{$exception->getMessage()}</code></p>";
        } else {
            $message[] = '<p><i>To reveal the error message, enable the <code>debug</code> configuration option.</i></p>';
        }

        $message[] = '<p>Either Redis server is unreachable, or the <code>WP_REDIS_CONFIG</code> constant in the <code>wp-config.php</code> file is incorrectly configured.</p>';
        $message[] = '<ul>';
        $message[] = '  <li>Is Redis server up and running?</li>';
        $message[] = '  <li>Is the correct <code>host</code>, <code>port</code> and <code>scheme</code> set?</li>';
